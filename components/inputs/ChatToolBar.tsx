...
const ChatToolbar = ({ onSend, style, roomId }: any) => {
    const [messageText, setMessageText] = useState('');
    const { createNewMessage } = useChatContext();

    const handleSend = () => {
        console.log('send pressed -->', messageText);
        if (messageText.trim().length > 0) {
            // const newMessage = {
            //     text: messageText,
            //     user: {
            //         _id: 1, // Or your user's ID
            //         name: 'Test User', // Or your user's name
            //     },
            // };
            // console.log('newMessage -->', newMessage);
            // onSend([newMessage]);
            // setMessageText('');
            createNewMessage(messageText, roomId, 'Brett');
            setMessageText('');
        }
    };
